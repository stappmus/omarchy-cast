...
if [[ "$source_dir" != "$target" ]]; then
  for file in manifest.json CastPanel.qml DelaySlider.qml backend.py streaming.py compatibility.py encoder.py omarchy-cast install.sh uninstall.sh README.md VALIDATION.md LICENSE icon.svg setup-network.sh; do
